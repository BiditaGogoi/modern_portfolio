...
const Navbar = () => {
  ...
  // Single unified theme color for the navbar
  const THEME_COLOR = "#c084fc"; // Sophisticated Light Purple

  const displayIndex = hoveredIndex !== null ? hoveredIndex : (activeIndex !== -1 ? activeIndex : 0);
  const displayColor = THEME_COLOR;
        ...
        <div className="hidden md:flex nav-wrapper pointer-events-auto">
          ...
          >
            {navLinks.map((link, index) => {
              ...
              // Exclusivity Logic: If something is hovered, only show its color.
              // Otherwise, show the active section's color.
              const itemColor = hoveredIndex !== null 
                ? (isHovered ? THEME_COLOR : "#94a3b8") 
                : (isActive ? THEME_COLOR : "#94a3b8");
